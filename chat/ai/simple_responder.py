...
class SimpleResponder:
    ...
    def _extract_city_from_message(self, message: str) -> str:
        """Извлечение города из сообщения через БД"""
        message_lower = message.lower().strip()
        
        for city_name in self.pricing.keys():
            if city_name.lower() in message_lower:
                logger.info(f"📍 Город найден в прайсе: '{city_name}'")
                return city_name
        
        # 2. Пытаемся найти через БД (get_city_chat)
        logger.debug(f"Пытаемся найти город '{message}' через БД...")
        city_obj = cities.api.get_city_chat(message)
        if city_obj:
            logger.info(f"📍 Город найден в БД: '{city_obj.name}'")
            return city_obj.name
        
        # 3. Если сообщение длинное, пытаемся найти паттерн "город X" или "г. X"
        if len(message) > 10:
            city_pattern = r'(?:город|г\.?)\s+([А-Яа-яЁё\-\s]+)'
            match = re.search(city_pattern, message)
            if match:
                potential_city = match.group(1).strip()
                city_obj = cities.api.get_city_chat(potential_city)
                if city_obj:
                    logger.info(f"📍 Город найден через паттерн: '{city_obj.name}'")
                    return city_obj.name
    ...
    def process(self, message: str, city: str = None, chat_id: str = None) -> str:
        ...
        if has_legal_keywords and not phone:
            return "Если вам нужно оказать услуги как для юридического лица и оплачивать вы будете на расчетный счет, то этим занимается отдельный менеджер, оставьте номер телефона и он в ближайшее время вам перезвонит и проконсультирует"
        
        if phone and not has_legal_keywords:
            deal_id = self._create_deal_regular(phone, city or "Не указан", message, chat_id)
            if deal_id == 'ERROR':
                return "Произошла ошибка при создании заявки. Пожалуйста, напишите нам позже ваш телефон."
            return f"Отлично! Заявка создана. Наш менеджер свяжется с вами в ближайшее время. Спасибо за обращение!"
        ...
        # Проверяем: город в прайсе?
        if city in self.pricing:
            min_price, min_hours = self._calculate_min_price(city)
            city_data = self.pricing[city]
            ppr = city_data.get('ppr', 700)  # цена за час за грузчика
            return f"Добрый день! Стоимость работы в городе {city}: от {min_price}₽ (минимум 2 грузчика на {int(min_hours)} часа, {ppr}₽/час за грузчика). Оставьте номер телефона для оформления заказа."
        
        # Город не в прайсе → проверяем БД
        logger.info(f"🔍 Город '{city}' не в прайсе, проверяем БД...")
        city_obj = cities.api.get_city_chat(city)
        
        if city_obj:
            # Город есть в БД → стандартные цены
            logger.info(f"✅ Город '{city}' найден в БД: {city_obj.name}")
            standard_price = 2 * 2 * 700  # 2 грузчика × 2 часа × 700₽/час = 2800₽
            return f"Добрый день! Стоимость работы в городе {city_obj.name}: от {standard_price}₽ (минимум 2 грузчика на 2 часа, 700₽/час за грузчика). Оставьте номер телефона для оформления заказа."
        
        # Нет ни в прайсе, ни в БД
        logger.info(f"❌ Город '{city}' не найден ни в прайсе, ни в БД")
        return f"Извините, в городе {city} мы пока не работаем. Оставьте номер телефона, и наш менеджер уточнит возможность выполнения заказа."
