...
import config
import report_bot.api


tg_bot = TeleBot(config.Production.REPORT_BOT_TOKEN)
...
def in_message(message: Message):
    ...
    tg_user = report_bot.api.get_user_by_tg_id(tg_id)
    if tg_user is None:
        if message.text != config.Production.REPORT_BOT_PASSWORD:
            tg_bot.send_message(tg_id, 'Введите пароль')
            return
        else:
            report_bot.api.create_tg_user(tg_id)
...
def send_message(text, audio=None):
    tg_users = report_bot.api.get_users()
    for user in tg_users:
        try:
            if audio is None:
                tg_bot.send_message(user.tg_id, text, disable_web_page_preview=True, parse_mode='HTML')
            else:
